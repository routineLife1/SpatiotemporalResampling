...
from models.model_union_2.softsplat import softsplat as warp
...
class AnimeInterp(nn.Module):
    ...
    def forward(self, I1, I2, reuse_things, merged, t):
        ...
        F1tdd = F.interpolate(F1t, scale_factor = 0.5, mode="bilinear", align_corners=False) * 0.5
        ...
        F2tdd = F.interpolate(F2t, scale_factor = 0.5, mode="bilinear", align_corners=False) * 0.5
        ...
        F1tddd = F.interpolate(F1t, scale_factor = 0.25, mode="bilinear", align_corners=False) * 0.25
        Z1ddd = F.interpolate(Z1, scale_factor = 0.25, mode="bilinear", align_corners=False)
        ...
        F2tddd = F.interpolate(F2t, scale_factor = 0.25, mode="bilinear", align_corners=False) * 0.25
        Z2ddd = F.interpolate(Z2, scale_factor = 0.25, mode="bilinear", align_corners=False)
        ...
        It_warp,x05 = self.synnet(torch.cat([I1t, I2t], dim=1), torch.cat([feat1t1, feat2t1], dim=1), torch.cat([feat1t2, feat2t2], dim=1), torch.cat([feat1t3, feat2t3], dim=1), merged)
